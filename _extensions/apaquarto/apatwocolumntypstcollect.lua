...
local function note_marker(note)
  return pandoc.RawBlock("typst", "// apaquarto-wide-note:" .. quarto.base64.encode(note))
end

local function note_text(el)
  local attrs = attributes(el)
  if attrs and attrs["apa-note"] then
    return stringify(attrs["apa-note"])
  end
end
...
function Pandoc(doc)
  ...
  for _, block in ipairs(doc.blocks) do
    ...
    local note = note_text(block)
    if id then
      blocks:insert(marker(id))
    end
    blocks:insert(block)
    if block.t == "Table" and note then
      blocks:insert(note_marker(note))
    end
  end
